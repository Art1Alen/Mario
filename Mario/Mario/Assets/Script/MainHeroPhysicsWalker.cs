...
namespace PlatformerMVC
{
    public class MainHeroPhysicsWalker
    {
        ...
        public void FixedUpdate()
        {
            ...
            if (walks &&
                (_goSideWay > 0 || !_contactsPoller.HasLeftContacts) && (_goSideWay < 0 || !_contactsPoller.HasRightContacts))
            {
            ...
            if (_contactsPoller.IsGrounded && _doJump && Mathf.Abs(_view.Rigidbody2D.velocity.y) <= _jumpThresh)
            {
            ...
            if (_contactsPoller.IsGrounded)
            {
                ...
                _spriteAnimatorController.StartAnimation(_view.SpriteRenderer, track, true,
                    _animationsSpeed);
            }
            else if (Mathf.Abs(_view.Rigidbody2D.velocity.y) > _flyThresh)
            {
                var track = AnimState.JumpDown;
                _spriteAnimatorController.StartAnimation(_view.SpriteRenderer, track, true, _animationsSpeed);
            }
        }
